function h=error_ellipse_adv(varargin)
...
if r==3 & c==3
  ...
  h1=plot3(x0+k*x,y0+k*y,z0+k*z,'color',prop.style);hold on
  [y,z,x] = getpoints(Cyz,prop.clip);
  h2=plot3(x0+k*x,y0+k*y,z0+k*z,'color',prop.style);hold on
  [z,x,y] = getpoints(Czx,prop.clip);
  h3=plot3(x0+k*x,y0+k*y,z0+k*z,'color',prop.style);hold on
...
elseif r==2 & c==2
  ...
  plot(scale*(x0+k*x),scale*(y0+k*y),'color',style, 'LineWidth', 5); hold on
%   h1=plot(scale*(x0+k*x),scale*(y0+k*y),prop.style); hold on
%   set(h1,'zdata',z+1)
%   if nargout
%     h=h1;
%   end
